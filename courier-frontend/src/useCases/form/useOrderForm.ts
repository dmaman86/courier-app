...
export const useOrderForm = (orderId: number | null) => {
    ...
    const handleSubmit = () => {
        ...
        if(order){
            if(isClient){
                ...
                isValid = validateForm() && v2;
            }else{
                isValid = (isAdmin) ? (order.currentStatus !== null) && (order.couriers.length > 0) : (order.currentStatus !== null);
            }
        }
